<form
    ...
    action="{{ route('users.byUserId.put', request()->userId) }}"
    method="POST"
    ...
    <label class="form-control col-span-12 w-full">
        <div class="label">
            <h5 class="label-text text-base font-semibold">
                @lang('pages.users.edit.formFields.name')
                :
            </h5>
        </div>
        <input
    ...
    <label class="form-control col-span-12 w-full">
        <div class="label">
            <h5 class="label-text text-base font-semibold">
                @lang('pages.users.edit.formFields.employeeId')
                :
            </h5>
        </div>
        <input
    ...
    <label class="form-control col-span-12 w-full">
        <div class="label">
            <h5 class="label-text text-base font-semibold">
                @lang('pages.users.edit.formFields.email')
                :
            </h5>
        </div>
        <input
    ...
    <label class="form-control col-span-12 w-full">
        <div class="label">
            <h5 class="label-text text-base font-semibold">
                @lang('pages.users.edit.formFields.roles')
                :
            </h5>
        </div>
        <select
            ...
            <optgroup label="--- {{ Str::upper(__('globals.pickOne')) }} ---">
                @foreach ($roles as $role)
                   <option
                    ...
                    {{ $user->roles->first()?->name ?? 'Pilih Role' }}
                </option>
                @endforeach
            </optgroup>
        </select>
